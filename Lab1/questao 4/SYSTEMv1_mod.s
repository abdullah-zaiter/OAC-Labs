...
ecallException:     addi    sp, sp, -264              # Salva todos os registradores na pilha
    ...
    # Zera os valores dos registradores temporarios e salvos presentes no RISC-V e ausentes no MIPS - 2015/1
    add     t0, zero, zero
    add     t1, zero, zero
    add     t2, zero, zero
    add     t3, zero, zero
...
#    beq     t0, a0, goToSDread
    ...
    beq     t0, a0, goToPopEvent


endSyscall:	lw	x1, 0(sp)  # recupera QUASE todos os registradores na pilha
#   lw	    x2,   4(sp)	# a0 retorno de valor
#   lw 	    x3,   8(sp)	# s8 retorno de valor
...
#   lw	    x5,  16(sp)      	# a1 
    ...
    lw      x30,  116(sp)
    lw      x31,  120(sp)
#   flw    $0,   124(sp) 	# fa0 retorno de valor
    flw    f1,  128(sp)
    flw    f2,  132(sp)
    flw    f3,  136(sp)
    flw    f4,  140(sp)
...
goToExitDE2:	j       goToExitDE2     ########### ecall 10 ou 110

goToPrintInt:	jal ra,     printInt               	# chama printInt
		j       endSyscall

goToPrintString: jal ra,     printString           	# chama printString
    		j       endSyscall

goToPrintChar:	jal ra,     printChar		# chama printChar
    		j       endSyscall

goToPrintFloat:	jal ra,     printFloat		# chama printFloat
    		j       endSyscall

goToReadChar:	jal ra,     readChar              	# chama readChar
    		j       endSyscall

goToReadInt:   	jal ra,     readInt                 # chama readInt
    		j       endSyscall

goToReadString:	jal ra,     readString              # chama readString
    		j       endSyscall

goToReadFloat:	jal ra,     readFloat               # chama readFloat
		j       endSyscall

goToPrintHex:	jal ra,     printHex                # chama printHex
		j       endSyscall
	
goToMidiOut:	jal ra,     midiOut                 # chama MIDIout
    		j       endSyscall

goToMidiOutSync:     	jal ra,     midiOutSync   	# chama MIDIoutSync
    			j       endSyscall

#goToSDread:     jal ra,     sdRead                  # Chama sdRead
#    		j       endSyscall
...
goToSleep:	jal ra,     sleep                  	# chama sleep
		j       endSyscall

goToRandom:	jal ra,     random                 	# chama random
    		j       endSyscall

goToCLS:	jal ra,     clsCLS                 	# chama CLS
    		j       endSyscall
...
loop2printInt:	lw 	t2, 0(sp)			# le digito da pilha
		...
		jal 	ra, printString			# chama o print string
...
IMPRIMIVEL:	li	a0, NUMCOLUNAS		# Num colunas 320
	mul 	t4, a0, a2			# t4 = coordenada y
    					
    	add     t4, t4, a1               	# t4 = 320*y + x
    	addi    t4, t4, 7                 	# t4 = 320*y + (x+7)
    	la      s11, VGAADDRESSINI          	# Endereco de inicio da memoria VGA
    	add     t4, t4, s11               	# t4 = endereco de impressao do ultimo pixel da primeira linha do char
...
# s8 = num de caracteres digitados	#
# 2017/2                		#
#########################################
# muda a0, s8, a0 e fp  
...
readString: 	addi 	sp, sp, -4			# reserva espaco na pilha
		sw 	ra, 0(sp)			# salva ra
		li 	s8, 0				# zera o contador de caracteres digitados
    		la      fp, LabelScanCode      	# Endereco da tabela de scancode inicial para readChar
    		
loopreadString: beq 	a1, s8, fimreadString   	# buffer cheio fim
		mv 	s9, ra			# salva ra
		jal ra, 	readChar			# le um caracter do teclado (retorno em a0)
		mv 	ra, s9  			# recupera ra
		beq 	a0, 0x0A, fimreadString	# se for tecla ENTER fim
		sb 	a0, 0(a0)			# grava no buffer
		addi 	s8, s8, 1			# incrementa contador
...
fimreadString: 	sb 	zero, 0(a0)			# grava NULL no buffer
		addi 	a0, a0, -1			# Para que a0 tenha o endereco do ultimo caractere digitado
		lw 	ra, 0(sp)			# recupera ra
		addi 	sp, sp, 4			# libera espaco
		jr 	ra				# retorna
...
readInt: 	addi 	sp,sp,-4		# reserva espaco na pilha
	...
	jal ra, 	readString			# le uma string de ate 10 digitos, s8 numero de digitos
	mv 	t0, a0			# copia endereco do ultimo digito
	li 	t2, 10				# dez
	li 	t3, 1				# dezenas, centenas, etc
	mv 	a0, zero			# zera o numero
	
loopReadInt: 	beq	s8,zero, fimReadInt	# Leu todos os digitos
...
ehposprintFloat: sb 	t0, 0(s0)			# coloca sinal no buffer
		...
		# Encontra o expoente em t0
		 mfc1 	t0, fa0			# recupera o numero float
		...
		mfc1 	t1, fa0			# recupera o numero float 
		...
		abs.s 	fa0, fa0		# fa0 recebe o m�dulo  de x
		lui 	t0, 0x3F80
...
loop2printFloat:  	c.lt.s 	0, f4, f10			# resultado eh < que 10? entao fim
		...
		# Neste ponto tem-se no flag 1 se fa0<1, em f3 o expoente de 10 e fa0 0 m�dulo do numero e s1 o sinal
...
fimprintFloat:		jal ra, 	printString			# imprime a string em a0
			lw 	ra, 0(sp)			# recupera ra
...
readFloat: addi sp, sp, -4			# aloca espaco
	...
	jal ra,	readString			# le string, retorna a0 ultimo endereco e s8 numero de caracteres
	mv 	s0, a0			# ultimo endereco da string (antes do \0)
	mv 	s1, s8			# numero de caracteres digitados
...
insere0AreadFloat: mv t0, s0		# endereco do ultimo caractere
		   addi s0, s0, 1		# desloca o ultimo endereco para o proximo
	   	   ...
	   	   mv s11, s7		# primeiro caractere
insere0Aloop:	   beq 	t0, s11, saiinsere0AreadFloat	# chegou no inicio entao fim
...
intreadFloat:		mtc1 	zero, f2			# zera parte inteira
			...
			mv 	s11, s7			# Primeiro Endereco
loopintreadFloat: 	blt 	t0, s11, fimintreadFloat	# sai se o enderefo for < inicio da string
			...
			add.s 	fa0,fa0,f2			# soma no resultado
...
#		li 	a0, 14
#		ecall		
#		blt 	a0, zero, FimSD	# se a0 <0 entao deu erro
#
#		mv 	a0, t3		# recupera o descritor
#		li 	a0, 16			# fecha o arquivo
#		ecall		
#		li 	a0, 0			# retorna sem erro
#		j 	FimSD			# retorna
#
#
...
#    		j       sdBusy
#    
# FimSDRead:	li      a0, 0                          # Sucesso na transferencia.         NOTE: Hardcoded. Um teste de falha deve ser implementado.
# 
# FimSD:		jr      ra				# retorna
############################################
...
############################################
time: 	DE2(timeDE2)
	li 	a0,30				# Chama o ecall do Mars
	ecall
	j 	fimTime				# saida
...
############################################
sleep: DE2(sleepDE2)
	li 	a0, 32				# Chama o ecall do Mars
	ecall			
	j 	fimSleep			# Saida
...
random: DE2(randomDE2)		
	li 	a0,41			# Chama o ecall do Mars
	ecall	
	j 	fimRandom		# saida
