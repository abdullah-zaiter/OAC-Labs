...
intprintFloat:		fmul.s 		f4, f4, f2		# ajusta o numero
		  	...
		  	fcvt.s.w	f5, s10
		  	flt.s		s10, f4, f5
		  	beqz		s10, JAFLOOR1							#		Substituicao da funcao FLOOR
		  	addi		s10, s10, -1
		  	fcvt.s.w	f5, s10	#######################################################################################################################
...
loopfracprintFloat:  	beq 		t1, zero, fimfracprintFloat	# fim dos digitos?
		  	...
		  	fcvt.s.w	f5, s10
		  	flt.s		s10, f4, f5
		  	beqz		s10, JAFLOOR2								#	Substituicao da funcao FLOOR
		  	addi		s10, s10, -1
		  	fcvt.s.w	f5, s10	#######################################################################################################################
	JAFLOOR2:	fmv.x.s		s10, f5				#cvt.s.w 	f5, f5			# parte inteira	; funcao do mars trocada por par fmv.x.s + mcvt.s.w
			fcvt.s.w	f5, s10
...
fimfracprintFloat: 	li 	t0,'E'			# carrega 'E'
			...
			addi 	s0, s0, 1		# incrementa endereco
		  	...
		  	li 	t0, '+'				# carrega '+'
		  	beqz 	s9, expposprintFloat			# nao eh negativo?
		  	li 	t0, '-'				# carrega '-'
...
fimreadString: 	sb 	zero, 0(a2)			# grava NULL no buffer
		#addi 	$v0, a2, -1			# Para que $v0 tenha o endereco do ultimo caractere digitado
		mv	a1, t0
		mv 	a0, a2
		lw 	ra, 0(sp)			# recupera $ra
		addi 	sp, sp, 4			# libera espaco
...
sleepDE2:	li 	t0, STOPWATCH			# endereco do StopWatch
...
naoehReadInt:			# gera erro "instruçao" invalida
